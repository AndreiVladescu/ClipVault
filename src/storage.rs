...
pub fn compact_history_log() -> anyhow::Result<()> {
    ...
    let tmp = path.with_extension("tmp");
    {
        ...
        for (key, agg) in items {
            let put: LogRec = LogRec::Put {
                key: key.clone(),
                ts: agg.created_ts,
                content: agg.content.clone(),
            };
            serde_json::to_writer(&mut out, &put)?;
            out.write_all(b"\n")?;
            if agg.last_ts > agg.created_ts {
                let touch: LogRec = LogRec::Touch {
                    key,
                    ts: agg.last_ts,
                };
                serde_json::to_writer(&mut out, &touch)?;
                out.write_all(b"\n")?;
            }
        }
        out.flush()?;
...
pub fn load_history_mru() -> anyhow::Result<Vec<ClipboardEntry>> {
    ...
    for line in reader.lines() {
        ...
        match rec {
            LogRec::Put { key, ts, content } => {
                let e: &mut (ClipboardContent, DateTime<Utc>) =
                    map.entry(key).or_insert((content, ts));
                if ts > e.1 {
                    e.1 = ts;
                }
            }
